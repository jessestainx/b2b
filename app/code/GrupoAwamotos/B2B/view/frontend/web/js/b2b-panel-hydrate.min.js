...
(function () {
    ...
    function buildPanelHtml(data)
    {
        ...
        var fullDisplay = formatDisplayName(data.full_name);
        var firstDisplay = formatDisplayName(data.first_name);
        var normId = function (s) {
            return String(s || '').trim().replace(/\s+/g, ' ').toLowerCase();
        };
        var nameEqualsCompany = companyDisplay && fullDisplay && normId(fullDisplay) === normId(companyDisplay);
        var displayNameRaw = (nameEqualsCompany && firstDisplay) ? firstDisplay : (fullDisplay || firstDisplay);
        var showCompanyLine = !!(companyDisplay && normId(displayNameRaw) !== normId(companyDisplay));
        var displayName = esc(displayNameRaw);
        var line2Raw = companyDisplay || data.group_name || '';
        var companyStartsWithFirst = !!(firstDisplay && companyDisplay
            && normId(companyDisplay).indexOf(normId(firstDisplay)) === 0);
        var line2Trigger = line2Raw;
        if (companyDisplay && (companyDisplay.length > 24 || companyStartsWithFirst)) {
        ...
        var triggerAria = 'Ol\u00e1, ' + firstDisplay;
        if (line2Trigger) {
            triggerAria += ' \u2014 ' + line2Trigger;
        }
        ...
        return (
            ...
            '<button type="button" class="b2b-status-trigger" aria-expanded="false" aria-controls="b2b-status-dropdown" aria-haspopup="true" aria-label="' + esc(triggerAria) + '">' +
            '<span class="b2b-status-trigger__icon" aria-hidden="true">' + icon('user') + '</span>' +
            '<span class="b2b-status-trigger__text" aria-hidden="true">' +
            ...
            (showCompanyLine ? '<span class="user-company">' + icon('building') + company + '</span>' : '') +
