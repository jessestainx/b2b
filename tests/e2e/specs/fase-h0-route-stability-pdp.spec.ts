/**
 * Fase H0.1 — Route Stability Investigation — PDP real
 * ========================================================
 * Investiga BUG-H0-CRIT-02 (ver docs/visual-qa/fase-h0-header-bug-report-2026-07-08.md):
 * uma PDP real nunca atinge `domcontentloaded` em Chromium headless (confirmado
 * com timeout de 90s no bug report original).
 *
 * PDP usada: mesma da reproducao original do bug (BUG-H0-CRIT-02) e da spec
 * fase-h0-header-functional-qa.spec.ts (rota `pdp`), para manter consistencia
 * entre as fases e permitir comparacao direta de evidencia.
 *
 ...
 *     specs/fase-h0-route-stability-pdp.spec.ts --project=h0-route-stability-1440-headless
 */
...
const ROUTE_PATH = '/bagageiro-titan-150-09-13-modelo-preto-macico-3000.html';
const ROUTE_LABEL = 'PDP real — bagageiro Titan 150 (BUG-H0-CRIT-02)';
const SPEC_SLUG = 'pdp';
  ...
  for (const variation of VARIATIONS) {
    test(`Variacao ${variation.id} — ${variation.label}`, async ({ browser }, testInfo) => {
      ...
      for (const step of STEPS) {
        ...
        console.log(
          `[H0.1-pdp][${variation.id}][${step.waitUntil}] fired=${result.fired} `
